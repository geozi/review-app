package gr.aueb.cf.reviewapp.service.dto.registration;

import lombok.AllArgsConstructor;
import lombok.Getter;
import lombok.NoArgsConstructor;
import lombok.Setter;
import org.springframework.stereotype.Component;
import gr.aueb.cf.reviewapp.controller.UserRestController;
import gr.aueb.cf.reviewapp.service.dto.insertion.UserInsertDTO;

/**
 * The {@link UserRegBadDataDTO} class is returned by the
 * {@link UserRestController} to the frontend if the received {@link UserInsertDTO}
 * contains invalid data. It inherits from the {@link RegBaseDTO} abstract class.
 * @author geozi
 * @version 1
 */
@Component
@NoArgsConstructor
@AllArgsConstructor
@Getter
@Setter
public class UserRegBadDataDTO extends RegBaseDTO{
    private Boolean regStatus = false;
    private String message = "Invalid field data";
}
